    ...
    <main className="min-h-screen bg-bg text-text-primary">
      <header className="fixed inset-x-0 top-0 z-40 border-b border-border bg-bg/85 backdrop-blur-md">
        <div className="mx-auto flex h-20 max-w-[1400px] items-center justify-between px-6 md:px-12">
          <Link
            to="/systems"
            className="font-mono text-xs uppercase tracking-[0.2em] text-text-secondary transition-colors hover:text-white"
          >
            ← Systems archive
          </Link>
          <a
      ...
      <section className="relative overflow-hidden px-6 pb-20 pt-40 md:px-12 md:pb-28 md:pt-48">
        <div
          className="pointer-events-none absolute left-1/2 top-1/2 h-72 w-[70vw] max-w-4xl -translate-x-1/2 -translate-y-1/2 rounded-full opacity-10 blur-3xl"
        ...
        >
          ...
          <p className="mt-10 max-w-2xl text-xl leading-relaxed text-text-secondary md:text-2xl">
            {project.subtitle}
          </p>
        </motion.div>
      </section>
      ...
      <section className="border-y border-border">
        <div className="mx-auto grid max-w-[1400px] grid-cols-2 px-6 md:grid-cols-4 md:px-12">
          {project.metrics.map((metric) => (
            <div
              key={metric.label}
              className="border-border px-4 py-10 odd:border-r md:border-r md:px-8 md:last:border-r-0"
            >
              <div className="text-3xl font-bold md:text-4xl">
                {metric.value}
                {metric.suffix}
              </div>
              <div className="mt-3 font-mono text-[10px] uppercase tracking-[0.2em] text-text-secondary">
                {metric.label}
              </div>
            </div>
          ))}
          <div className="px-4 py-10 md:px-8">
            <div className="flex flex-wrap gap-2">
              {project.tech.slice(0, 3).map((tech) => (
                <span key={tech} className="border border-border px-2 py-1 text-xs">
                  {tech}
                </span>
              ))}
            </div>
      ...
      <section className="border-y border-border bg-surface px-6 py-24 md:px-12 md:py-36">
        <div className="mx-auto max-w-[1400px]">
          ...
          <p className="max-w-4xl text-2xl leading-relaxed text-text-secondary md:text-3xl">
          ...
          <div className="mt-16 grid gap-5 md:grid-cols-3">
            {project.features.map((feature, index) => (
              <motion.article
                ...
                className="border border-border bg-bg p-8"
              >
      ...
      <section className="mx-auto max-w-[1100px] px-6 py-24 md:px-12 md:py-36">
        <div
          className="mb-14 font-mono text-xs uppercase tracking-[0.2em]"
          style={{ color: project.accent }}
        >
          Process
        </div>
        <ol className="border-t border-border">
          {project.timeline.map((step, index) => (
            <li
              key={step.step}
              className="grid gap-4 border-b border-border py-8 md:grid-cols-[80px_1fr_2fr]"
            >
      ...
      <section className="border-t border-border px-6 py-24 md:px-12 md:py-36">
        <div className="mx-auto max-w-[1400px]">
          ...
          <p className="mt-8 max-w-4xl text-3xl font-bold leading-tight md:text-5xl">
            {project.results}
          </p>
